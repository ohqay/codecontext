...
struct IgnoreRules: Sendable {
    ...
    init(
        ...
        if respectGitIgnore {
            self.gitignorePatterns = Self.parseIgnoreFile(at: rootPath + "/.gitignore")
        } else {
        ...
        if respectDotIgnore {
            self.dotignorePatterns = Self.parseIgnoreFile(at: rootPath + "/.ignore")
        } else {
            self.dotignorePatterns = []
        }
    }
...
private struct IgnorePattern {
    ...
    init(pattern: String) {
        ...
        self.isDirectoryOnly = self.pattern.hasSuffix("/")
        
        // Check if pattern is rooted (starts with /)
        self.isRooted = self.pattern.hasPrefix("/")
        
        // Check for double asterisk (**) patterns
        self.hasDoubleAsterisk = self.pattern.contains("**")
    }
