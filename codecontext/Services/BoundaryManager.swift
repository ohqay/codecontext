...
enum BoundaryManager {
    ...
    nonisolated static func cleanForDisplay(_ xml: String) -> String {
        // Match START markers (always followed by newline) and END markers (not followed by newline)
        let startPattern = "<!--\(appIdentifier)-[A-Z]+-START-[A-Z0-9]+-->\n"
        let endPattern = "<!--\(appIdentifier)-[A-Z]+-END-[A-Z0-9]+-->"
        ...
        guard let startRegex = try? NSRegularExpression(pattern: startPattern, options: []),
            ...
            logDebug("Failed to create regex for boundary cleanup", details: "Patterns: \(startPattern), \(endPattern)")
            return xml
        }

        let range = NSRange(location: 0, length: xml.utf16.count)
        
        // First remove START markers with their newlines
        let afterStartRemoval = startRegex.stringByReplacingMatches(in: xml, options: [], range: range, withTemplate: "")
        
        // Then remove END markers (without newlines)
        let finalRange = NSRange(location: 0, length: afterStartRemoval.utf16.count)
        let result = endRegex.stringByReplacingMatches(in: afterStartRemoval, options: [], range: finalRange, withTemplate: "")
