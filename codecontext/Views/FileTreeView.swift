...
struct FileTreeView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        tokenColumn.width = 65
        tokenColumn.minWidth = 55
        tokenColumn.maxWidth = 75
    ...
    func updateNSView(_ nsView: NSScrollView, context: Context) {
        ...
        let currentSnapshot = StateSnapshot.from(
            fileTreeModel: fileTreeModel, filterText: filterText)
    ...
    private func determineUpdateType(current: StateSnapshot, previous: StateSnapshot?) -> UpdateType
    {
        ...
        if current.rootNodeId != previous.rootNodeId || current.allNodeIds != previous.allNodeIds {
            return .fullReload
        }
    ...
    private func performFullReload(outlineView: NSOutlineView, coordinator: Coordinator) {
        ...
        if scrollPosition > 0 {
            DispatchQueue.main.async {
                outlineView.enclosingScrollView?.documentView?.scroll(
                    NSPoint(x: 0, y: scrollPosition))
            }
        }
    }
    ...
    private func expandMatchingItems(_ outlineView: NSOutlineView, node: FileNode) {
        for child in node.children {
            if child.isDirectory {
                let hasMatching = hasMatchingDescendant(
                    child, filter: filterText.trimmingCharacters(in: .whitespacesAndNewlines))
                if hasMatching {
        ...
        QLPreviewPanelDataSource, QLPreviewPanelDelegate
    {
        ...
        private func makeCheckboxView(for node: FileNode) -> NSView {
            ...
            let checkbox = NSButton(
                checkboxWithTitle: "", target: self, action: #selector(checkboxToggled(_:)))
        ...
        private func refreshSelectionStatesForNode(
            _ node: FileNode, in outlineView: NSOutlineView, row: Int? = nil
        ) {
            ...
            if let checkboxView = outlineView.view(
                atColumn: 2, row: nodeRow, makeIfNecessary: false),
        ...
        func updateExpansionStates(in outlineView: NSOutlineView) {
            ...
            let shouldBeExpanded = Set(
                parent.fileTreeModel.allNodes.filter { $0.isExpanded }.map { $0.id })
        ...
        func previewPanel(_ panel: QLPreviewPanel, previewItemAt index: Int) -> QLPreviewItem {
            return currentPreviewFile?.url as? QLPreviewItem ?? URL(fileURLWithPath: "/")
                as QLPreviewItem
        }
